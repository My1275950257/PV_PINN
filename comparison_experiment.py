...
运行前提：
   python data_generator.py   → 生成 ./data/train_dataset.npz
   python train.py            → 生成 pinn_model_v3.pth
"""
...
def build_classifiers(random_seed=RANDOM_SEED):
    ...
    return [
        ...
        ('PINN-RF(本文)', RandomForestClassifier(n_estimators=100, max_depth=6,
...
def run_accuracy_experiment(X_raw, X_raw_noisy, X_phys, X_phys_noisy, Y):
    ...
    clf_configs = [
        ...
        ('MLP',          MLPClassifier(hidden_layer_sizes=(256,128,64), max_iter=500,
                         ...
                         X_tr_rs,  X_te_rs,  X_tr_rsn, X_te_rsn),
        ('PINN-RF(本文)', RandomForestClassifier(n_estimators=100, max_depth=6, random_state=RANDOM_SEED, n_jobs=-1),
...
def plot_dual_accuracy(res_clean, res_noisy):
    ...
    plt.tight_layout()
    plt.savefig('fig_dual_accuracy.svg', format='svg', bbox_inches='tight')
    plt.savefig('fig_dual_accuracy.png', dpi=200, bbox_inches='tight')
...
def plot_robustness_drop(res_clean, res_noisy):
    ...
    ax.spines[['top', 'right']].set_visible(False)
    plt.tight_layout()
    plt.savefig('fig_robustness_drop.svg', format='svg', bbox_inches='tight')
    plt.savefig('fig_robustness_drop.png', dpi=200, bbox_inches='tight')
...
def plot_few_shot(few_shot_results, sample_sizes):
    ...
    ax.spines[['top', 'right']].set_visible(False)
    plt.tight_layout()
    plt.savefig('fig_few_shot.svg', format='svg', bbox_inches='tight')
    plt.savefig('fig_few_shot.png', dpi=200, bbox_inches='tight')
...
def plot_per_class_heatmap(res_noisy):
    ...
    plt.tight_layout()
    plt.savefig('fig_per_class_heatmap.svg', format='svg', bbox_inches='tight')
    plt.savefig('fig_per_class_heatmap.png', dpi=200, bbox_inches='tight')
...
def plot_radar(res_clean, res_noisy, few_shot_results, timings):
    ...
    plt.tight_layout()
    plt.savefig('fig_radar.svg', format='svg', bbox_inches='tight')
    plt.savefig('fig_radar.png', dpi=200, bbox_inches='tight')
...
def plot_pinn_confusion(res_noisy):
    ...
    plt.tight_layout()
    plt.savefig('fig_pinn_confusion_noisy.svg', format='svg', bbox_inches='tight')
    plt.savefig('fig_pinn_confusion_noisy.png', dpi=200, bbox_inches='tight')
