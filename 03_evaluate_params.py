...
model.load_state_dict(torch.load('pinn_model_v3.pth', map_location=device, weights_only=True))
model.eval()
...
plt.savefig(figure_path('Rs_prediction.svg'), format='svg', bbox_inches='tight')
...
# 保存为 SVG 矢量图
plt.savefig(figure_path('Rsh_prediction.svg'), format='svg', bbox_inches='tight')
plt.show()
